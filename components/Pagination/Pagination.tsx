...
export default function Pagination({ pageCount }: PaginationPropsType) {
      ...
      <div className="flex justify-center">
        <ul className="flex w-fit items-center justify-center border-primary">
          ...
          <li className="flex flex-wrap divide-x-2 border-x-2">
            {[...Array(Math.ceil(pageCount / 10))].map((e, index) => {
              return (
                <div
                  onClick={() => {
                  ...
                  className={`flex h-8 w-8 cursor-pointer items-center justify-center border-t-2 border-b-2 md:h-12 md:w-12 $`}
                >
                  {index + 1}
                </div>
              );
